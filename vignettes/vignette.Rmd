...
The Bayesian Melding model is applied in spatial statistic. It melds the output from the deterministic simulating model with the point-referenced data from the monitoring locations using a hierarchical Bayesian framework [@liu2011empirical].
...
We can use the following code to replicate our simulation studies. It allows for parallel computing and takes about half an hour to run the most complicated case with grid cell equal 50 on a 80-core machine. The host name of the server I used to run the function is `cpu152.math.private`.
...
We simply load the results to avoid from knitting the file for a long time. The results can be downloaded from https://www.dropbox.com/scl/fi/1zzkfoex7ouoy4t9x5dn3/running_results.RData?rlkey=c8648z0fuvj4do8a1802c5d23&dl=0 . 
...
We add a checking of the input and output of this package, especially for the main function `run_func``. The following are some examples for testing:

Avoid from invalid `m1`. 

```{r}
try(run_func(m1 = 1.1, m2 = 2, n.pred = 100, ran = 5, nm = 1, nhours = 2,
                     ton = 100, burnin = 50, degree = 1, nbeta = 3,
                     ntheta = 2, nab = 2, nerror = 2, cov.model = "exponential", seed = 42,
                     num_for_cores = 1))
```

Avoid from invalid `m2`. 

```{r}
try(run_func(m1 = 1, m2 = 2.1, n.pred = 100, ran = 5, nm = 1, nhours = 2,
                     ton = 100, burnin = 50, degree = 1, nbeta = 3,
                     ntheta = 2, nab = 2, nerror = 2, cov.model = "exponential", seed = 42,
                     num_for_cores = 1))
```

Avoid from invalid `n.pred`. 

```{r}
try(run_func(m1 = 1, m2 = 2, n.pred = 100.1, ran = 5, nm = 1, nhours = 2,
                     ton = 100, burnin = 50, degree = 1, nbeta = 3,
                     ntheta = 2, nab = 2, nerror = 2, cov.model = "exponential", seed = 42,
                     num_for_cores = 1))
```
